...
namespace MonolithEngine
{
    ...
    public class UserInterface
    {
        ...
        public void AddUIElement(IUIElement newElement)
        {
            ...
            if (newElement.GetChildren().Count > 0)
            {
                foreach (IUIElement child in newElement.GetChildren())
                {
                    if (child is SelectableUIElement)
                    {
                        (child as SelectableUIElement).SetUserInterface(this);
                    }
                }
            }
        }
